...
class KGEnvironment:
    def __init__(self,uri):
        self.uri = uri #"bolt://139.19.6.57:7687"  # "bolt://127.0.0.1:7687"
        self.driver = GraphDatabase.driver(self.uri, auth=(
            "neo4j", NEO4J_PASSWORD), encrypted=False,  max_connection_lifetime=3600*24*30, keep_alive=True)
        ...
        with self.driver.session() as session:
            try:
                result = session.read_transaction(
                    self.__fetch_one_hop_nodes, nodeid)
            except Exception as e:
                print(e)
                print(
                    "Warning: An exception occured in the neo4j database; an empty list will be returned!")
                return []
            return result
    ...
    def get_number_of_neighbors(self, nodeid):
        with self.driver.session() as session:
            result = session.read_transaction(
                self.__fetch_number_of_neighbors, nodeid)
            return result
    ...
    def __fetch_one_hop_nodes(self, tx, nodeid):
        ...
        for record in tx.run("MATCH (n:e {nodeid: '" + nodeid + "'}) CALL apoc.path.expandConfig(n, {   minLevel: 1, maxLevel: 1}) YIELD path RETURN nodes(path) as nodes, relationships(path) as relations LIMIT 1000;"):
            relation = re.split(
                "!|\||_", record['relations'][0]['rel'].strip("!"))
            paths.append([record['nodes'][0]['nodeid'],
                         relation, record['nodes'][1]['nodeid']])
