...
def _standard_normalize(values, axes=(0,)):
    ...
    normalized_values = ((values - values_mean) /
                         (tf.sqrt(values_var) + epsilon))
...
@gin.configurable
class QAReinforceAgent(ReinforceAgent):
    """A REINFORCE Agent.
    ...
    def __init__(self,
        ...
        super(QAReinforceAgent, self).__init__(
    ...
    def _train(self, experience, weights=None, fractions=None):
        ...
        with tf.GradientTape() as tape:
            loss_info = self.total_loss(
                experience, tf.stop_gradient(returns),  weights=weights, fractions=fractions,
                training=True)
    ...
    def total_loss(self, experience, returns, weights=None, fractions=None, training=False):
        ...
        policy_state = _get_initial_policy_state(
            self.collect_policy, time_steps)
        actions_distribution = self.collect_policy.distribution(
            time_steps, policy_state=policy_state).action

  #      print("action dist: ", actions_distribution)
 #       print("experience act: ", experience.action)
        policy_gradient_loss,loss_fractions = self.policy_gradient_loss(
        ...
        self.setLossFractions(loss_fractions)
      ...
      #  print("entropy reg loss: ", entropy_regularization_loss)

        network_regularization_loss = tf.nn.scale_regularization_loss(
            self._actor_network.losses)

       # print("networ reg los: ", network_regularization_loss)
        ...
        losses_dict = {
            ...
            #'loss_per_question':loss_fractions
        ...
        # Total loss not in loss_info_extra.
    ...
    def setLossFractions(self,loss_fractions):
        self.lossFractions = loss_fractions

    def getLossFractions(self):
        return self.lossFractions
    ...
    def policy_gradient_loss(self,
        ...
        #print("action log prob: ", action_log_prob_times_return, flush=True)
       # print("policy gradien loss: ", policy_gradient_loss, flush=True)

        loss_parts = []
        size = action_log_prob_times_return.shape[1]

        #print("fractions: ", fractions)
        if not fractions is None:
            maxval = max(fractions)

            #print("size: ", size, "maxval: ", maxval)
            for i in range(0,maxval+1):
                loss_parts.append(0.0)
                for j in range(size):
                    if fractions[j]==i:
                        #print("loss parts before: ", loss_parts[i])
                        #print("action log prob: ", action_log_prob_times_return[j])
                        loss_parts[i] = tf.math.add(tf.cast(loss_parts[i], dtype=tf.float32), action_log_prob_times_return[0,j])
                        #print("loss parts: ", loss_parts[i])
               # print("loss parts of ", i, ": ", loss_parts[i])
                loss_parts[i] = tf.math.divide(loss_parts[i], policy_gradient_loss) 
                #print("loss parts of ", i, " after div: ", loss_parts[i])
            # for i in range(len(fractions)):
            #     if i == 0:
            #         loss_parts.append(-tf.reduce_sum(input_tensor=action_log_prob_times_return[0,0:i])/ policy_gradient_loss)
            #     else:
            #         loss_parts.append(-tf.reduce_sum(input_tensor=action_log_prob_times_return[0,i:i+1])/ policy_gradient_loss)
        ...
        #print("final loss: ", policy_gradient_loss, flush=True)
        return policy_gradient_loss, loss_parts
    ...
    def entropy_regularization_loss(self, actions_distribution, weights=None):
        ...
        if self._entropy_regularization:
            loss = _entropy_loss(actions_distribution,
                                 self.action_spec, weights)
