...
start_time="$(date -u +%s)"
python qa/CONQUER/eval_QA.py --data $DATA --checkpointpath $CHECKPOINTQA --epochs 20 --resultpath $RUN_NAME >> $RUN_NAME"eval_qa.log" || exit 1
end_time="$(date -u +%s)"
elapsed="$(($end_time-$start_time))"
...
start_time="$(date -u +%s)"
python qa/CONQUER/eval_QA.py --data $GPTEVALDATA --checkpointpath $CHECKPOINTQA --topepoch 15 --test --flag "gpt"  --resultpath $RUN_NAME >> $RUN_NAME"eval_qa_gpt.log" || exit 1
end_time="$(date -u +%s)"
elapsed="$(($end_time-$start_time))"
echo $elapsed "seconds needed for QA inference"
