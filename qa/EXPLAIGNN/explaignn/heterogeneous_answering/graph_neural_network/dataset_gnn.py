...
class DatasetGNN(Dataset):
    ...
    def prepare_data(config, data_path, train=False):
        ...
        print("error cases: ")
        print("no answer: ", DatasetGNN.noAnsCount)
        print("existential count: ", DatasetGNN.existentialCount)
        print("max  ent reached: ", DatasetGNN.maxEntCount)
        print("too many evidences: ", DatasetGNN.tooManyEvidCount)
    ...
    def prepare_turn(config, turn, train=False):
        ...
        if train:
             ...
             #   "label"
            #].lower() in ["yes", "no"]:
            if not evaluation.answer_presence(evidences, gold_answers)[0]:
                DatasetGNN.noAnsCount += 1
                raise ContinueWithNext("No answer present in top evidences.")
            elif gold_answers[0]["label"].lower() in ["yes", "no"]:
                DatasetGNN.existentialCount += 1
                raise ContinueWithNext("Existential question.")
