...
class GNNModule(HeterogeneousAnswering):
    ...
    def train1RefPerEpoch(self, sources=("kb", "text", "table", "info"), train_path=None, dev_path=None):
        """
        ...
        """
        self.logger.info(f"Loading data using 1 ref per epoch...")
        ...
        for epoch in range(epochs):
            train_path = os.path.join(
                data_dir, qu, ers, sources_str, f"train_ers-{method_name}_" +str(epoch) +"ref.jsonl"
            )
    ...
    def train(self, sources=("kb", "text", "table", "info"), train_path=None, dev_path=None):
        ...
        self.logger.info(f"Config: {json.dumps(self.config, indent=4)}")
        if self.config.get("ref_per_epoch", False):
            return self.train1RefPerEpoch(sources, train_path, dev_path)
