...
class IterativeGNNs(HeterogeneousAnswering):
    ...
    def inference_on_turns(self, turns, sources=("kb", "text", "table", "info"), train=False):
        ...
        for i in range(len(self.config["gnn_inference"])):
            ...
            # compute ans pres
            turns_with_answers = list()
            if "answers" in turns[0]:
                for turn in turns:
                    if  "answer_presence" in turn.keys():
                        turns_with_answers.append(turn)
                    else:
                        print("turn without answerpresence: ", turn)
                   
            
            
            answer_presence_list = [turn["answer_presence"]  for turn in turns_with_answers]
            ...
            self.gnns[i].inference_on_turns(turns_with_answers)

            turns_before_iteration.append(copy.deepcopy(turns_with_answers))

        # remember top evidences
        for turn_idx, turn in enumerate(turns_with_answers):
