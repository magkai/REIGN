...
from explaignn.question_understanding.structured_representation.srm_reign import (
	REIGNStructuredRepresentationModule,
)
...
class Pipeline:
    ...
    #     self.logger.debug(f"len(input_data) {len(data)}")
        if "test_qu_path" in self.config.keys():
            output_path = self.config["test_qu_path"]
        else:
            output_path = f"{output_dir}/res_{self.name}_qu.json"
        if os.path.exists(output_path):
        # if os.path(output_path).isfile():
            with open(output_path, "r") as fp:
                data = json.load(fp)
            self.logger.info("QU was done earlier.")
        else:
       ...
       # with open(output_path, "r") as fp:
        ...
        for sources_str in source_combinations:
            ...
            sources = sources_str.split("_")
            #if "test_ers_path" in self.config.keys():
            #     output_path = self.config["test_ers_path"]
            #     self.logger.info("Already done ER.")
            # else:
           ...
           # output_path = f"{output_dir}/res_{self.name}_ers.json"
            ...
            # log result
            res_str = f"Gold answers - {sources_str} - P@1 ({num_questions}): {p_at_1}, absolute P: {sum(p_at_1_list)}"
            ...
            mrr = round(mrr, 3)
            # log result
            res_str = f"Gold answers - {sources_str} - MRR ({num_questions}): {mrr} , absolute mrr: {sum(mrr_list)}"
            self.logger.info(res_str)
            self.result_logger.info(res_str)
            ...
            # log result
            res_str = f"Gold answers - {sources_str} - H@5 ({num_questions}): {hit_at_5}, , absolute hit: {sum(hit_at_5_list)}"
            self.logger.info(res_str)
            self.result_logger.info(res_str)

    def run_gpt_eval_per_ref(self, sources_str, refId, clean_up=True):
        """
        Run the pipeline using gold answers for the next turns.
        `sources_str` can either be a single string, or a list of strings (several source combinations).
        The parameter `clean_up` controls whether the QU and ERS modules would
        ...
        input_dir = self.config["benchmark_path"]
        test_path = self.config["test_input_path"] + "_ref" + str(refId) + ".json"
        input_path = os.path.join(input_dir, test_path)
        with open(input_path, "r") as fp:
            data = json.load(fp)

        output_path = f"{output_dir}/res_{self.name}_qu_ref{str(refId)}.json"
        self.qu.inference_on_data(data)
        self.logger.info("Done with QU.")
        if clean_up:
            self.qu = None  # free up memory
        ...
        for sources_str in source_combinations:
            ...
            output_dir = self.set_output_dir(sources_str)
            sources = sources_str.split("_")
            output_path = f"{output_dir}/res_{self.name}_ers_ref{str(refId)}.json"
            #with open(output_path, "r") as fp:
             #   input_data = json.load(fp)
            self.ers.inference_on_data(input_data, sources)
            store_json_with_mkdir(input_data, output_path)
            ...
            self.ha.inference_on_data(input_data, sources)
            self.logger.info("Done with HA.")
            output_path = f"{output_dir}/res_{self.name}_gold_answers_ref{str(refId)}.json"
            store_json_with_mkdir(input_data, output_path)

            # compute results
            p_at_1_list = [turn["p_at_1"] if "p_at_1" in turn.keys() else 0 for conv in input_data for turn in conv["questions"] ]
            p_at_1 = sum(p_at_1_list) / len(p_at_1_list)
            p_at_1 = round(p_at_1, 3)
            num_questions = len(p_at_1_list)
            # log result
            ...
            # compute results
            mrr_list = [turn["mrr"] if "mrr" in turn.keys() else 0.0 for conv in input_data for turn in conv["questions"]]
            mrr = sum(mrr_list) / len(mrr_list)
            mrr = round(mrr, 3)
            # log result
            res_str = f"Gold answers - {sources_str} - MRR ({num_questions}): {mrr} , absolute mrr: {sum(mrr_list)}"
            ...
            # compute results
            hit_at_5_list = [turn["h_at_5"] if "h_at_5" in turn.keys() else 0 for conv in input_data for turn in conv["questions"]]
            hit_at_5 = sum(hit_at_5_list) / len(hit_at_5_list)
            hit_at_5 = round(hit_at_5, 3)
            # log result
            res_str = f"Gold answers - {sources_str} - H@5 ({num_questions}): {hit_at_5}, , absolute hit: {sum(hit_at_5_list)}"
    ...
    def run_gpt_eval_all(self, sources_str):
        """
        Run the pipeline using gold answers for the next turns.
        `sources_str` can either be a single string, or a list of strings (several source combinations).
        The parameter `clean_up` controls whether the QU and ERS modules would
        ...
        mrr = 0.0
        m_len = 0
        #we have 20 reformulations + the original question
        for i in range(21):
            input_dir = self.config["benchmark_path"]
            test_path = self.config["test_input_path"] + "_ref" + str(i) + ".json"
            input_path = os.path.join(input_dir, test_path)
            with open(input_path, "r") as fp:
                data = json.load(fp)

            output_path = f"{output_dir}/res_{self.name}_qu_ref{str(i)}.json"
            self.qu.inference_on_data(data)
            self.logger.info("Done with QU.")
            ...
            for sources_str in source_combinations:
                ...
                output_dir = self.set_output_dir(sources_str)
                sources = sources_str.split("_")
                output_path = f"{output_dir}/res_{self.name}_ers_ref{str(i)}.json"
                self.ers.inference_on_data(input_data, sources)
                store_json_with_mkdir(input_data, output_path)
                self.logger.info("Done with ER.")
                ...
                self.ha.inference_on_data(input_data, sources)
                self.logger.info("Done with HA.")
                output_path = f"{output_dir}/res_{self.name}_gold_answers_ref{str(i)}.json"
                store_json_with_mkdir(input_data, output_path)

                # compute results
                p_at_1_list = [turn["p_at_1"] if "p_at_1" in turn.keys() else 0 for conv in input_data for turn in conv["questions"]]
                p_at_1 += sum(p_at_1_list)
                p_len += len(p_at_1_list)
                p_at_1_curr = sum(p_at_1_list) / len(p_at_1_list)
                p_at_1_curr = round(p_at_1_curr, 3)
                num_questions = len(p_at_1_list)
                # log result
                res_str = f"Gold answers - {sources_str} - Reformulation ({str(i)}) - P@1 ({num_questions}): {p_at_1_curr}, absolute P: {sum(p_at_1_list)}"
                self.logger.info(res_str)
                self.result_logger.info(res_str)
                ...
                mrr +=  sum(mrr_list)
                m_len += len(mrr_list)
                mrr_curr = sum(mrr_list) / len(mrr_list)
                mrr_curr = round(mrr_curr, 3)
                # log result
                res_str = f"Gold answers - {sources_str} - Reformulation ({str(i)}) - MRR ({num_questions}): {mrr_curr} , absolute mrr: {sum(mrr_list)}"
                self.logger.info(res_str)
                self.result_logger.info(res_str)

                # compute results
                hit_at_5_list = [turn["h_at_5"] if "h_at_5" in turn.keys() else 0 for conv in input_data for turn in conv["questions"]]
                hit_at_5 += sum(hit_at_5_list)
                h_len += len(hit_at_5_list)
                hit_at_5_curr = sum(hit_at_5_list) / len(hit_at_5_list)
                hit_at_5_curr = round(hit_at_5_curr, 3)
                # log result
                res_str = f"Gold answers - {sources_str} - Reformulation ({str(i)}) - H@5 ({num_questions}): {hit_at_5_curr}, , absolute hit: {sum(hit_at_5_list)}"
                self.logger.info(res_str)
                self.result_logger.info(res_str)

        #final results:
        p_at_1_curr = p_at_1 / p_len
        p_at_1_curr = round(p_at_1_curr, 3)
        num_questions = p_len
        # log result
        res_str = f"Gold answers - {sources_str} - All Reformulations - P@1 ({num_questions}): {p_at_1_curr}"
        self.logger.info(res_str)
        self.result_logger.info(res_str)
        ...
        mrr_curr = round(mrr_curr, 3)
        # log result
        res_str = f"Gold answers - {sources_str} - All Reformulations - MRR ({num_questions}): {mrr_curr}"
        self.logger.info(res_str)
        self.result_logger.info(res_str)

        # compute results
        hit_at_5_curr = hit_at_5 / h_len
        hit_at_5_curr = round(hit_at_5_curr, 3)
        # log result
        res_str = f"Gold answers - {sources_str} - All Reformulations  - H@5 ({num_questions}): {hit_at_5_curr}"
        self.logger.info(res_str)
        self.result_logger.info(res_str)
    ...
    def collect_gpt_results(self, sources_str):
        p_at_1 = 0
        p_len = 0
        hit_at_5 = 0
        h_len = 0
        ...
        for i in range(21):
            ...
            if os.path.exists(output_path):
                with open(output_path, "r") as fp:
                    data = json.load(fp)
                    p_at_1_list = [turn["p_at_1"] if "p_at_1" in turn.keys() else 0 for conv in data for turn in conv["questions"]]
                    p_at_1 += sum(p_at_1_list)
                    p_len += len(p_at_1_list)
            
                    mrr_list = [turn["mrr"] if "mrr" in turn.keys() else 0.0  for conv in data for turn in conv["questions"]]
                    mrr +=  sum(mrr_list)
                    m_len += len(mrr_list)
            
                    hit_at_5_list = [turn["h_at_5"] if "h_at_5" in turn.keys() else 0  for conv in data for turn in conv["questions"]]
                    hit_at_5 += sum(hit_at_5_list)
                    h_len += len(hit_at_5_list)
        ...
        output_path = f"{output_dir}/res_{self.name}_gold_answers.json"
        store_json_with_mkdir(all_data, output_path)
    ...
    def _load_qu(self, use_gold_answers):
        ...
        if qu.startswith("nc_"):
            ...
            return REIGNStructuredRepresentationModule(self.config, use_gold_answers)
        elif qu == "qrew":
...
def main():
    ...
    elif function == "--gpt-eval":
        sources_str = sys.argv[3] if len(sys.argv) > 3 else "kb_text_table_info"
        pipeline = Pipeline(config, use_gold_answers=True)
        pipeline.run_gpt_eval_all(sources_str)

    elif function == "--collect-gpt-results":
        sources_str = sys.argv[3] if len(sys.argv) > 3 else "kb_text_table_info"
        pipeline = Pipeline(config, use_gold_answers=True)
        pipeline.collect_gpt_results(sources_str)

    elif function == "--gpt-eval_per_ref":
        sources_str = sys.argv[3] if len(sys.argv) > 3 else "kb_text_table_info"
        refId = sys.argv[4] if len(sys.argv) > 4 else "0"
        pipeline = Pipeline(config, use_gold_answers=True)
        pipeline.run_gpt_eval_per_ref(sources_str, refId)
