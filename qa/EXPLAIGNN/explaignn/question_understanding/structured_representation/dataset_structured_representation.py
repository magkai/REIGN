...
class DatasetStructuredRepresentation(torch.utils.data.Dataset):
    def __init__(self, config, tokenizer, path, train=False):
        self.config = config
        ...
        #MODIFIED for reign:
        if train:
            input_encodings, output_encodings, dataset_length = self._load_data_for_reign(path)
        else:
            input_encodings, output_encodings, dataset_length = self._load_data(path)
    ...
    def _load_data_for_reign(self, path):
        """
        Opens the file, and loads the data into
        a format that can be put into the model.
        ...
        for conversation in dataset:
           # history = list()
            for turn in conversation["questions"]:
                ...
                inputs.append(turn["history"] + " ||| " +turn['question'])
