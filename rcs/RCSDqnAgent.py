...
from RCSGreedyPolicy import RCSGreedyPolicy
...
@gin.configurable
class RCSDqnAgent(tf_agent.TFAgent):
    """A DQN Agent.
    ...
    def __init__(
        ...
        super(RCSDqnAgent, self).__init__(
            time_step_spec,
            action_spec,
            policy,
            collect_policy,
    ...
    def _setup_policy(self, time_step_spec, action_spec,
        ...
        policy = RCSGreedyPolicy(policy) #greedy_policy.GreedyPolicy(policy)
        ...
        target_policy = q_policy.QPolicy(
            ...
            observation_and_action_constraint_splitter=(
                self._observation_and_action_constraint_splitter))
        self._target_greedy_policy = RCSGreedyPolicy(target_policy)
    ...
    def _loss(self,
        ...
        with tf.name_scope('loss'):
            ...
            #cond = tf.equal(next_time_steps.step_type.numpy(), StepType.LAST)
            #print("cond: ", cond)
            #zeros = tf.zeros([next_q_values.shape[0]], dtype=tf.float32)
            #print("zeros: ", zeros)
          # print("bef: ", self.q_net(observations))
            #out = tf.where(cond, zeros , next_q_values)
            #print("out: ", out)
        ...
        #    print("reg loss: ", self._q_network.losses)
    ...
    def _compute_q_values(self, time_steps, actions, training=False):
      ...
      #  print("q network weights: ", self._q_network.trainable_weights)
  ...
  #      print("target network weights: ", self._target_q_network.trainable_weights)
 #       print("greedy action: ", greedy_actions)
        # Handle action_spec.shape=(), and shape=(1,) by using the multi_dim_actions
        # param. Note: assumes len(tf.nest.flatten(action_spec)) == 1.
        multi_dim_actions = tf.nest.flatten(self._action_spec)[
            0].shape.rank > 0
        return common.index_with_actions(
